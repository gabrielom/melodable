...
- **The window floor is what keeps the bar intact**: `minWidth` in `tauri.conf.json` is **1180**, measured as the narrowest width where every trainer control fits at natural size. The binding case is now **piano in sheet** (1173px) — `KEY`, `NOTE | DEG`, `COLOUR` and `ROLL | SHEET` all at once; piano in roll needs 1163 and pads only 941, so the longest *pads* title stopped being the constraint at handoff 11. It had come down from 1216 to 1052 as controls left for home, and handoff 11 §3 put it back up: three changes freed ~100px (theme off the trainer bar, gaps 9→7 and 11→8, one divider instead of two) and the new controls spent more than that. **A plain browser understates the floor** — the device chip reads "NO DEVICE" at 91px rather than its 116px cap, so force the widest label when measuring (`scratchpad/floor11.mjs` does). The bar's 84px left inset is part of the budget too; the frames use 72, and the extra 12 is ours (72 left the ✕ almost touching the zoom button). Below the floor **nothing is pushed out** — §3.2's shrink order takes over: the spacers collapse, then the device name truncates (min 46px, keeping the LED and the caret), then the lesson title (min 36px). The bar's own symptom is silent, so the check is `bar.scrollWidth <= bar.clientWidth`; jsdom has no layout, so that is a browser measurement and not a unit test.
...
- **The chord ribbon is derived, and it is a map rather than a score.**
  ...
  line is for. Same trade as `keySignatureFor`, and it fails the same honest
