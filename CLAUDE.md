...
- **The window floor is what keeps the bar intact**: `minWidth` in `tauri.conf.json` is 1192, measured as the narrowest width where all 19 trainer controls fit at natural size with the longest built-in name ("Syncopated Groove", full at 1181px). The bar's 84px left inset is part of that budget — widening it to clear the traffic lights pushed the floor up by exactly the same 12px, so re-measure whenever it changes. Nothing drops out responsively any more — if you add a control, re-measure and raise the floor, don't start hiding things. The lesson title is the one elastic element (`flex: 0 1 auto; min-width: 0`) so an imported clip with a long name ellipsises instead of pushing controls off the edge.
